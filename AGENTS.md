...
- Treat an applied preset as a named template over visible durable settings, not as a hidden parallel runtime configuration. Selecting a named preset must write its complete governed value set; selecting `Custom` must change nothing; manually changing any governed value while a named preset is selected must change only the selector to `Custom`; and runtime behavior must read the governed settings rather than branching on the preset. Make the stored defaults match the default named preset, order named choices from least to most intensive when that progression applies, and place `Custom` last when a compatible enum change is available.
- Apply a durable preset after config recovery and before subscribing to manual setting changes. Guard programmatic preset writes against recursively selecting `Custom`, unsubscribe handlers during teardown, and defer an optional FoA Mod Manager public `Refresh` call until the next frame so programmatic values replace cached drafts cleanly. Preserve raw config section and key identities when reorganizing display metadata, and keep a large preset's governed values in a small number of adjacent, clearly named display sections rather than one oversized tab.
- Applied preset selectors and their governed settings are durable configuration and must remain eligible for the normal typed recovery policy. Do not permanently exclude them as pseudo-buttons. Reserve one-shot preset triggers and their recovery exclusions for workflows that are inherently actions rather than editable saved profiles.
- Do not force applied-preset behavior onto selectors that remain runtime modes, quality tiers, archetypes, strengths, or profiles and do not represent a complete set of independently editable values. Document that distinction in the setting description when the name alone could imply an applied template.
- Every applied-preset owner must provide a focused source contract for its complete value matrix, `Custom` behavior, startup/event ordering, live-value runtime authority, and deferred optional FoA Mod Manager refresh. Add an in-game smoke row covering displayed value refresh, named-to-`Custom` edits, direct `Custom` selection, replacement by another named preset, and named and custom save/reload persistence. Keep `tools/Test-AppliedPresetContracts.ps1` updated to invoke every participating mod's focused contract.
...
- Ordinary Vortex staging means copying the exported package into the Vortex mods directory as a new folder for a new version and queuing truthful local grouping metadata for the Grailwright Nexus Metadata extension. The extension may attach the stable logical filename, local version, `grailwright-local` source, and configured real Nexus page ID so separate folders collapse under one Vortex mod row. It must not assign a Nexus file ID to unpublished bytes, edit profile enablement or deployment state, or edit `state.v2` directly.
- After a successful Nexus version upload, `Publish-NexusMod.ps1` may queue the exact staged release for the Grailwright Nexus Metadata Vortex extension. Promotion must use the immutable release receipt, verify the staged payload file-for-file against the exact uploaded archive, import or reuse that archive through Vortex's supported download APIs, and attach its Nexus source, page ID, numeric file ID, archive hash/size, logical filename, and archive link through supported Vortex actions. Never promote an unresolved, unverified, missing, or content-mismatched stage, never assign a published file ID to unpublished bytes, and never edit `state.v2` directly.
- Before creating or updating a Vortex collection, run `tools/Test-VortexCollectionReadiness.ps1`; every enabled Grailwright version included in the collection must be an exact promoted Nexus release rather than a `grailwright-local` test build.

...
Assert-Contract ($source.Contains('ExecutionHandoffProgress = 0.90f')) "Execution feedback does not hand off near the end of the finisher."
Assert-Contract ([regex]::IsMatch($source, 'targetDeathConfirmed[\s\S]*?progress01 >= ExecutionHandoffProgress[\s\S]*?StartKillingBlowHitMarker\(_pendingExecutionKillingBlow\)')) "Confirmed executions do not hand off to the buffered normal killing-blow marker."

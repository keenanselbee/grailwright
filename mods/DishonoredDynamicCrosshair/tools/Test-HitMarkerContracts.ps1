...
Assert-Contract ($source.Contains('_activeHitMarkerPlayerAttack = playerAttack;') -and $source.Contains('_activeHitMarkerPlayerAttack = feedback.PlayerAttack;')) "Regular and killing-blow markers do not both retain source priority."
...
Assert-Contract ([regex]::Matches($source, 'GetSizeUnitConversion\(').Count -eq 6) "Not every visual size path uses the shared size conversion."
...
Assert-Contract ($source.Contains('OnSteelAndBoneTargetedKillingBlowResolved')) "The target-aware killing-blow feedback receiver is missing."
...
Assert-Contract ($source.Contains('[BepInDependency("ks.tgfoa.killing-blow-mastery", BepInDependency.DependencyFlags.SoftDependency)]')) "Killing Blow Mastery is not an optional integration dependency."
Assert-Contract ($source.Contains('KillingBlowMastery.ExecutionVisualApi')) "The Killing Blow Mastery execution-visual API is not resolved."
Assert-Contract ($source.Contains('"TryGetKillingBlowQuality"')) "Execution previews do not query Steel and Bone's authoritative corpse quality."
Assert-Contract ([regex]::IsMatch($source, 'ReferenceEquals\(target, _executionPreviewTarget\)[\s\S]*?_pendingExecutionKillingBlow = feedback;')) "Killing-blow feedback is not buffered for the exact execution target."
Assert-Contract ([regex]::IsMatch($source, 'Color\.Lerp\(\s*Color\.white,\s*new Color32\(0x8C, 0x00, 0x03, 0xFF\)')) "Execution previews do not tint from white to killing-blow dark red."
Assert-Contract ([regex]::IsMatch($source, '_executionPreviewBaseImage\s*=\s*CreateHitMarkerOverlayImage\([\s\S]*?"DishonoredExecutionPreviewBase"[\s\S]*?_hitMarkerAssets\.TryGetValue\(\s*HitMarkerFrame\.Neutral')) "Execution previews do not retain a white neutral marker beneath the corpse-quality skull."
Assert-Contract ([regex]::IsMatch($source, 'phase == ExecutionPhaseActive\s*&& targetDeathConfirmed\s*&& !_executionHandoffStarted\s*&& _pendingExecutionKillingBlow != null\)[\s\S]*?StartKillingBlowHitMarker\(_pendingExecutionKillingBlow\)')) "Confirmed executions do not hand off immediately to the buffered normal killing-blow marker."
Assert-Contract (-not $source.Contains('ExecutionHandoffProgress')) "Execution handoff still depends on normalized animation progress."
Assert-Contract ([regex]::IsMatch($source, '!_executionPreviewActive\s*\|\| _executionHandoffStarted\)[\s\S]*?SetImageEnabled\(_executionPreviewBaseImage, false\);[\s\S]*?SetImageEnabled\(_executionPreviewImage, false\);')) "Execution preview layers can remain above the confirmed killing-blow marker after handoff."
Assert-Contract ([regex]::IsMatch($source, 'if \(!_executionHandoffStarted\)\s*\{\s*color\.a \*= UnderlyingCrosshairOpacityMultiplier\(\);\s*\}')) "The normal killing-blow marker remains hidden behind the fading execution preview."
Assert-Contract ([regex]::IsMatch($source, 'phase == ExecutionPhaseCompleted[\s\S]*?if \(targetDeathConfirmed[\s\S]*?ClearExecutionPreview\(\);')) "Completed or cancelled execution previews are not cleared safely."
...
Assert-Contract ($steelSource.Contains('public const int ApiVersion = 7;')) "Steel and Bone's feedback API is not v7."
...
Assert-Contract ([regex]::IsMatch($steelSource, 'public static event Action<object, int, float, float, bool, bool, bool, bool, bool, string, float>\s+TargetedKillingBlowResolved;')) "Steel and Bone's target-aware killing-blow event is missing."
Assert-Contract ($steelSource.Contains('public static bool TryGetKillingBlowQuality(')) "Steel and Bone's pre-death quality query is missing."

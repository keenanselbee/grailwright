...
Add-Result "Outdoor Wyrdnight roaming" (@($states | Where-Object { $_.State -eq "Roaming" -and $_.Reason -eq "None" }).Count -gt 0) "state=Roaming"
Add-Result "Protected outdoor Wyrdnight" (@($states | Where-Object { $_.State -eq "Roaming" -and $_.Protection -eq "protected" }).Count -gt 0) "Roaming protection=protected"
Add-Result "Exposed outdoor Wyrdnight" (@($states | Where-Object { $_.State -eq "Roaming" -and $_.Protection -eq "exposed" }).Count -gt 0) "Roaming protection=exposed"

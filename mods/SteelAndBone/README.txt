...
DamageNumberMode defaults to AllDamage for the existing floating-number presentation. ResistAndImmuneOnly replaces numbers with RESISTED or IMMUNE on every qualifying direct hit, while ResistAndImmuneOnlyOnce shows each of those notices only once per enemy. The notice-only modes ignore neutral hits, weaknesses, and damage-over-time ticks. DamageNumbersEnabled remains the master switch for all built-in floating combat text.

...
ModifyParryWindowBonus = true
PositiveParryWindowBonusMultiplier = 0.75
...
Avalon AI Overhaul is conditionally compatible. Disable ModifyEnemySightRange, ModifyEnemyHearingRange, and ModifyEnemyAggroPersistence in Steel and Bone so Avalon can control enemy perception and pursuit without stacking modifiers. Its camp response, investigation, and alert behavior do not otherwise overlap. When both mods and Grail Floating Text are active, Steel and Bone warns at the main menu only for matching modifiers that are currently effective.
...
Steel and Bone says nothing when no overlap is active. With Grail Floating Text installed, a confirmed overlap produces one short warning-styled System notice at the main menu and lists the exact conflicting Steel and Bone toggles in BepInEx/LogOutput.log.

...
Version 3.8.9 adds configurable floating-combat-text modes. AllDamage preserves the complete number presentation, ResistAndImmuneOnly reports only qualifying direct-hit resistance and immunity, and ResistAndImmuneOnlyOnce reports resistance and immunity separately once per enemy. Notice-only modes exclude damage-over-time ticks and do not affect optional crosshair hit markers.

Version 3.9.0 makes potion use more forgiving while extending the meaningful overdrinking window. Every potion adds 40 buildup to its matching class, two same-class potions are safe, and a third triggers when the first-to-third span is within 5/10/15 seconds across Tempered, Hardened, and Crucible.
...
Version 4.1.0 turned the former late-game Progressive Tenacity layer into a campaign-wide preset mechanic. Its campaign factor is `lerp(0.40, 1.00, clamp((hero level - 1) / 34, 0, 1))`, so it is present at level 1 without overwhelming the opening and reaches full campaign strength at level 35. Native NPC type supplies the class maximum: 12% for Trash, 18% for Normal, 30% for Elite, 38% for MiniBoss, and 50% for Boss enemies. Tempered, Hardened, and Crucible then apply factors of 0.75, 1.00, and 1.25. Critters and Hero Summon targets receive none.

Host Resolve scales only MiniBoss and Boss Tenacity. The first qualifying summon is free. Summons two through eight evenly interpolate from x1.00 to maximum factors of x1.50 for MiniBosses and x1.75 for Bosses. When the 80% Tenacity cap limits a raw endpoint, the implementation normalizes the current factor against that class maximum and interpolates the available headroom across the same linear curve. This preserves the one- and eight-summon endpoints while ensuring every qualifying summon through the eighth contributes predictably. A qualifying actor must be a living, active, hero-owned native `NpcHeroSummon` in the target's scene and within 50 meters. One shared snapshot is refreshed at most once per second instead of enumerating the summon host for every target. This identity contract gives direct Soul and Service compatibility without a hard dependency or mod-specific summon registry.

Tenacity applies at full strength to player-caused poise, force, and enemy stamina damage, including the direct and parry routes that can drive native stamina stagger. Native hero-owned summons count as player-caused sources, while Hero Summon targets remain exempt. Direct non-damage-over-time health damage uses half strength as a bounded burst brake without changing enemy maximum health. The effective value is capped at 80%, after which a confirmed native or Steel and Bone material weakness or confirmed weak spot halves Tenacity once for that hit. Critical, backstab, and generic damage bonuses do not, and critical damage receives no separate penalty.

Version 4.2.1 extends half-strength Tenacity to harmful buildup caused by the hero or a hero-owned summon. Status Tenacity is capped at 60%, preserving at least 70% of every contribution while native per-status thresholds remain authoritative. The shared `CharacterStatuses.BuildupStatus` route handles new and continuing buildup, and a narrow transactional `PersistentAoE.ApplyBuildupStatus` scope restores the owner that native source construction omits. Positive buildup, direct `AddStatus`, forced `CompleteBuildup`, active duration, strength, decay, and tick damage remain unchanged. Damage over time, environmental damage, and unrelated NPC combat are excluded. The complete system adds no adaptive performance tracking, stagger immunity, saved NPC state, dynamic health or damage scaling, material-feedback reclassification, Host Resolve toggle, or Host Resolve notification.
...
| Broad armor and AI rewrites | Steel and Bone narrowly scales native armor penalties, Light mobility, and arrow velocity while leaving player armor protection native; enemy AI and armor classification remain native. |
...
Steel and Bone presets are independent from the vanilla difficulties `Story`, `Easy`, `Normal`, `Hard`, `Survival`, and `Challenge`. They are applied templates: selecting a named preset writes its 23 governed values into visible live settings, while editing one of those settings changes the selector to `Custom`. System toggles and unrelated expert or presentation settings remain untouched.
...
Presets should be a general matchup-strength and difficulty influence, not separate rulesets. Every Steel and Bone rule has one base multiplier. The applied material intensity scales that multiplier toward or away from neutral: Tempered is closer to vanilla, Hardened uses the base rule, and Crucible makes the same rule more decisive. Vanilla-authored multipliers use the separate applied amplification value: Tempered leaves them unchanged by default, while Hardened and Crucible amplify their distance from neutral with clamps.

For the 3.0 global layer, Tempered applies 5% incoming, outgoing, and experience pressure while keeping resource, armor, poise, recovery, and enemy movement neutral. Hardened applies 10% damage and experience pressure plus the existing 5% supporting profile and one additional enemy attack slot. Crucible applies 15% damage and experience pressure plus the existing 10% supporting profile and two additional slots. Positive combat mana regeneration, accumulated positive critical-damage bonuses above the native +0.45 baseline, and accumulated positive parry-window bonuses use 1.00/0.75/0.50 across Tempered, Hardened, and Crucible. Dash stamina cost uses 1.00/1.15/1.30 and stacks with the game's resolved cost. Enemy sight, hero footstep hearing range, and native aggro persistence use x1.20/x1.40/x1.60 across Tempered, Hardened, and Crucible; arrows retain x1.10/x1.30/x1.50. Potion Poisoning adds 40 buildup in each independent class bucket and decays at 4/2/1.333 points per second, so two same-class potions are safe and a third triggers within 5/10/15 seconds; Health, Mana, and Stamina triggers drain 30% of the matching maximum while Utility drains 15% of all three. Standard food uses 0.50/0.375/0.25 health rate, x4 duration, and 1 discrete stamina point per second. Hostile archer aim scatter uses 1.50/1.25/1.00 meters by preset through the game's native target-point inaccuracy, keeping Crucible archers dangerous without perfectly centered aim. Confirmed weak spots add 10%, 20%, or 30% base damage by preset. Outgoing and incoming player damage remain independently toggleable, while their exact values come from the applied live settings.
...
| Critical and weak-spot tuning | Preserves the native `0.45` critical bonus, scales only accumulated positive critical-damage bonus above it by `1.00`/`0.75`/`0.50`, and adds `0.10`/`0.20`/`0.30` beside native precision components on confirmed weak spots before outgoing pressure and material matchups. Hero and item stats remain untouched. | Keep. Deliberate hit placement offsets preset pressure while excess critical scaling no longer overwhelms the mid-game curve. |
...
| Implemented item | Current behavior | Keep or change |
|---|---|---|
| `DamageNumbersEnabled` and `DamageNumberMode` | The master toggle controls built-in floating combat text. AllDamage shows the complete outgoing number presentation; ResistAndImmuneOnly shows only direct-hit RESISTED and IMMUNE notices; ResistAndImmuneOnlyOnce shows each notice separately once per enemy. | Keep. Notice-only modes exclude damage-over-time ticks and do not affect optional hit markers. |
...
|---|---|---|
| `Preset` | Implemented as an applied template with `Custom`. | Main player-facing selector; writes the governed live values from one central matrix and leaves independent toggles and expert settings untouched. |
...
| Feedback controls | Implemented through `DamageNumbersEnabled` and `DamageNumberMode`. | Controls the built-in all-damage, resistance/immunity-only, and once-per-enemy floating-text routes for outgoing player hits. |
...
| Potion overdrinking pass | Potion healing and presentation remain native; direct restoratives feed independent Health, Mana, and Stamina buckets, all other consumed potions feed Utility, and each potion adds 40 buildup with preset decay of 4/2/1.333 points per second. Two same-class potions are safe; a third triggers within 5/10/15 seconds. Completing one clears every bucket and activates the single native status with a 30% matching-resource drain, or a 15% all-resource drain for Utility. |

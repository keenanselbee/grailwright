...
Necrotic is not part of this native enum taxonomy. Steel and Bone treats it as an exact semantic subtype only when Soul and Service API v5 or newer confirms that the current Generic Magical `Damage` instance is Soul Rend. It is never inferred from a spell name, Soul Salvage item identity, or Soul Claim.
...
| `surfaceType` | Useful material cue: most enemies are `HitFlesh`, skeletons are `HitBones`, constructs/golems are often `HitStone`, and Banshee/Melancholy/Wyrdspawn are often `HitMagic`. Steel and Bone uses `HitBones`, `HitStone`, and `HitWood` for body-material weapon reactions without treating them as creature identity. `HitFlesh` and `HitMagic` remain diagnostic context because they overlap too many families. |
...
| Flamegobbler | 3 templates; `Fire 0%`. | Fire immunity is confirmed. Steel and Bone adds an exact 115% Cold weakness as a readable opposing-element counter. |
...
| Stagfather variants | Fire golems have `Fire 33%` and `Cold 133%`; ice golems have `Cold 33%` and `Fire 133%`; electric golems have `Electric 33%`; base Stagfather has no vanilla multiplier. | Elemental variants are corrected to Construct despite their bone/zombie metadata, retain their native opposing-element reactions, and gain the focused fire-body Wet rule where appropriate. Steel and Bone adds Poison 133% to the electric golem only. |
| Wyrdspawn and Wyrdspirits | Wyrdspawn commonly inherits `Abstract:WyrdnessBound`; Wyrdspirits also do. No default `Wyrdness` resistance multiplier was found on their templates. | Use the abstract as a target-family detector and leave neutral templates neutral; preserve authored native reactions where present. |
...
| `DrownedZombie` | Drowners and similar drowned undead | Confirmed Drowner pattern: `Fire 50%`, `Status_Blind`; Steel and Bone adds blood/bleed and mild pierce resistance while leaving slash neutral | `Electric` and blunt as Steel and Bone overlays; do not assume fire weakness |
| `BoneUndead` | Skeletons and dry undead | Bleed immunity, poison low/zero, slash/pierce resistance, and Steel and Bone `Cold 66%` where native data is neutral | Confirmed: `Bludgeoning 133%`; Cold spells can retain independent Chill buildup; holy/silver and fire are not confirmed engine subtypes |
| `FleshUndead` | Zombies, bloody undead, Wights, Scourge-type enemies | Bleed/poison/blood depending template plus mild pierce resistance; slash remains neutral | Fire, holy/silver item-term, blunt only where it reads well |
...
| `Spirit` | Shades, ghosts, mist, banshees | Bleed/poison immunity where status refs confirm it; mild physical resistance | Wyrdness; holy/silver or purge only if later detectable |
| `Construct` | Golems, wrought, animated armor | Bleed/poison/blood low and Steel and Bone `Cold 66%` where native data is neutral; respect per-template elemental reactions such as Lost Knight `Fire 50%`, Cairnguard `Cold 50%`, or Forgeborn `Cold 150%` | `Electric` for Lost Knight/Tibby-style targets; `Bludgeoning` only as Steel and Bone overlay unless confirmed |
| `Wyrd` | Wyrdspawn, Wyrdspirits, Wyrdheir, Wyrd-bound enemies | `Abstract:WyrdnessBound` identifies the family; neutral templates receive no blanket Wyrdness resistance | Exact physical or elemental counters where supported; native reactions otherwise |
...
| 5 | Wailcap | Sea Creature / Animal | `SeaFlesh` plus fungal-body flag | Confirmed: `Poison 25%`; Cold through the Sea family when native is neutral | x1.15 Necrotic through fungal-body evidence; Electric, slash, and pierce through SeaFlesh | Exact correction preserves SeaFlesh for general rules while making living fungal matter Necrotic-vulnerable. |
| 5 | Viridian Wailcap | Sea Creature / Animal | `SeaFlesh` plus fungal-body flag | Follows the Wailcap Sea identity when its runtime name is exposed | x1.15 Necrotic; Electric, slash, and pierce through SeaFlesh | Exact Wailcap matching covers the named variant. |
| 5 | Swarm Of Viridian Wailcaps | Sea Creature / Animal | `SeaFlesh` plus fungal-body and swarm delivery flags | Sea-family Cold and individual arrow resistance | x1.15 Necrotic; Electric, slash, and non-arrow area attacks | Keep non-Necrotic rules mild because swarm enemies can become tedious. |
...
| 2 | Corpse Eater | Creature / Humanoid | `Flesh` | Light poison or disease | Fire, bleed, slash | The summon and large-template gaps now receive the family's confirmed Fire 120% and Wyrdness 80% reactions. |
...
| 2 | Grindylow | Creature / Humanoid | `Wyrd` or `Flesh` | Confirmed: `Wyrdness 80%` | Confirmed: `Cold 120%`; bleed/slash as overlay | Game templates are `Abstract:Monster`, not zombie/sea abstracts. Steel and Bone supplies the missing 120% Cold reaction on `Grindylow_Summon` before SeaFlesh can apply its opposing Cold rule. |
...
| 3 | Ogre | Creature / Humanoid | `Flesh` | Blunt at 90% | Pierce 115%; blood, bleed, and poison 110% | Exact living-brute rules; ordinary Flesh remains the base family. |
| 3 | Singworm | Creature / Animal | `Flesh` | Blunt 85% | Slash 115% | Exact soft-body rule on both extracted Singworm templates. |
...
| 3 | Fleshtree | Creature / Flora | `Flora` | Bleed, poison, pierce | Fire, slash; axes 120% | The axe rule replaces the ordinary 115% Slash rule and does not stack. |
...
| 3 | Mistbearer | Spirit / Nature | `Spirit` | Wyrdness 80% | Fire 120% | Exact base and mimic rules follow the established Mistling pattern. |
| 3 | Strawchild / Straw Son | Spirit / Nature | `Spirit` with bone body | Spirit biological rules; bone-body weapon rules | Fire 120%, slash 115% on the exact Straw Son template | Exact identity correction prevents `HitBones` from making it Bone Undead. |
| 3 | Strawfather / Straw Dad | Spirit / Nature | `Spirit` with bone body | Spirit biological rules; bone-body weapon rules | Fire 120%, slash 115% on the exact Straw Dad template | Exact identity correction; native reactions remain authoritative. |
...
| 3 | Wyrdslime | Wyrdtwisted / Flora | `Wyrd` | Blunt 80%; no vanilla damage multiplier found | Steel and Bone Cold 110%; Slash and Pierce remain neutral | Exact soft-body rule makes its formless mass absorb impact without displacing Cold as the best answer. |
| 3 | Wyrdspawn | Wyrdtwisted / Human | `Wyrd` | Confirmed family marker: `Abstract:WyrdnessBound`; no blanket Wyrdness resistance | Slash 110% | Exact modest Slash rule; authored native reactions remain authoritative. |
...
| 3 | Wyrdheir | Wyrdtwisted | `Wyrd` | Cold 60% | Fire, holy/silver item-term, pierce | The challenge variant now receives the ordinary variant's missing Cold resistance. |
...
| 3 | Skeleton Mage | Undead / Skeleton | `BoneUndead` | Confirmed skeleton pattern: bleed immunity and `Bludgeoning 133%`; Steel and Bone supplies `Cold 66%` where native data is neutral | Blunt confirmed; Cold spells can retain independent Chill buildup; holy/silver item-term or `Electric` only as overlay | Good test for anti-undead plus anti-caster behavior. |
...
| 4 | Lost Knight | Construct / Golem | `Construct` | Confirmed: `Poison 50%`, `Fire 50%`, bleed immunity; Steel and Bone adds `Cold 66%` | Confirmed: `Electric 200%`; blunt as overlay | Existing construct logic preserves the very strong vanilla Electric weakness while independent Chill buildup remains available. |
...
| 4 | Electric Stagfather Golem | Construct / Golem | `Construct` | Confirmed Electric 33% | Poison 133%; blunt through Construct | Exact variant rule completes its counter without weakening constructs broadly; all elemental Stagfather golems are corrected to Construct. |
...
| 4 | Flamegobbler | Undead / Draugr | `FleshUndead` or `FireFlesh` | Confirmed: `Fire 0%` | Steel and Bone `Cold 115%`; holy/silver item-term and blunt remain deferred | Exact Flamegobbler matching adds the Cold counter without creating a broad fire-body family. |
| 4 | Frostbitten Warrior | Undead / Draugr | `FleshUndead` | Cold 75%; normal flesh-undead biological and pierce resistances | Fire 115%; blunt through the flesh-undead family | Exact correction prevents `HitStone` from making it a construct. |
...
| 4 | Blood Abomination | Formless blood entity | Exact soft body; no broad family | Confirmed Wyrdness 80%; Blunt 80% | Confirmed Cold 120%; authored Slash 120% (110% after elite clamp) | Exact correction clears misleading `Abstract:BoneMask` so BoneUndead physical rules do not override its fluid body. |
| 4 | Wight | Wyrdtwisted / Flora | `Flora` | Poison, bleed, and pierce resistance through Flora | Fire and slash through Flora | Exact correction prevents `HitBones` and zombie metadata from replacing its Wyrd-flora identity. |
...
| 4 | Sleepwalker | Wyrdtwisted / Human | `Wyrd` with stone body | Confirmed: `Cold 75%`, `Poison 75%`; stone-body edge and arrow resistance | Confirmed: `Fire 133%`; Blunt through stone body | `HitStone` controls physical material without granting Construct biological resistance. |
| 4 | Ancient Beholder | Creature / Humanoid | `Flesh` | Blunt 90% | Pierce 112%; blood, bleed, and poison 108% | Exact correction prevents its `HitStone` surface from applying Construct rules to a living horror. |
| 4 | Crimson Creeper | Creature / Animal | `Flesh` | Poison, bleed if carapaced | Fire, blunt, pierce | Verify carapace/creeper visuals. |
| 4 | Frostgrot | Creature / Animal | `Flesh` | Cold 75% | Fire 115% | Exact runtime-name rule follows its explicit frost identity; native reactions remain authoritative if a template is discovered. |
| 4 | Frost Wolf | Creature / Animal | `Flesh` | Cold 75% | Fire 115% | Exact summoned Frost Wolf variants only; ordinary wolves are unchanged. |
| 4 | Rootambusher | Creature / Flora | `Flora` | Poison, bleed, pierce, arrows | Fire, slash; axes 120% | Exact correction makes the `HitWood` body authoritative over misleading zombie/bloody metadata. |
...
| 5 | Rimefiend | Special, Cold enemy | `ColdSpecial` | Confirmed: `Cold 60%` | Fire 120% before elite clamp | Exact Fire answer; template is Tier 5, level 100 Elite. |
| 5 | Lir's summoned tentacle | Sea creature / appendage | `Flesh` | Blunt 85% | Slash 115% | Exact soft-body rule; does not generalize to all sea creatures. |
...
| 5 | Tidewraith | Sea Creature / Flora | `SeaFlesh` | Confirmed Cold 60%; Blunt 90% | Electric plus existing mild Slash and Pierce overlays | Exact soft-body resistance complements the flexible aquatic plant identity without treating it as a spirit. |
...
| 6 | Tibby | Construct / Golem | `Construct` | Bleed/blood/poison and `Cold 66%` as overlays | Confirmed: `Electric 133%`; blunt as overlay | Named MiniBoss; native reactions remain authoritative and independent Chill buildup remains available. |
...
| 5 | Nivera The Chillheart, The Frostbound Consort | `ColdSpecial` | Confirmed Nivera: `Cold 50%`; bleed/poison only if undead/spirit | Nivera Fire 133%; `Electric`, holy/silver item-term | Exact Fire answer for Nivera; elite clamp remains authoritative. |
...
| 6 | Senan The Giant | Giant `Flesh` | Existing Flesh rules only | Blood, poison, edges through ordinary Flesh | `Abstract:Giant` now overrides misleading `HitStone` construct classification without adding a new giant-only rule. |
...
|---:|---|---|---|---|
| 1 | Bone undead | Implemented; validate in game | `Skeleton`, `Bone`, `Abandoned Warrior`, `Skeleton Mage`, `Abstract:Skeleton`, `HitBones` | Current rules resist blood/bleed, Cold, slash/pierce, and generic physical; blunt is a small Steel and Bone weakness only when vanilla has not already handled it. Confirmed skeletons take x0.40 Necrotic, but broad BoneMask or bone-body evidence alone does not qualify. |
| 1 | Drowned zombies | Implemented; validate in game | `Drowner`, `Drowned`, `Drowned Knight`, `Ghost Crew`, `Scourge`, `Abstract:Zombie` plus water/dead naming | Current rules resist blood/bleed and mildly resist pierce while leaving slash neutral; Electric/blunt are the shared weaknesses and Necrotic is x0.60. Confirm that vanilla Drowner `Fire 50%` remains preserved. |
| 1 | Constructs and animated armor | Implemented; validate in game | `Golem`, `Construct`, `Sentinel`, `Forgeborn`, `Cairnguard`, `Lost Knight`, `Tibby`, `Bottomless`, `Brimshade`, `HitStone` | Current rules resist blood/bleed/poison, Cold, slash/pierce, generic physical, and especially Necrotic at x0.25; blunt is a Steel and Bone weakness. Confirm template-specific elemental exceptions still win through vanilla skip logic and Chill remains independent. |
| 1 | Wyrd | Implemented; validate in game | `Wyrd`, `Wyrdspawn`, `Wyrdspirit`, `Wyrdslime`, `Wyrdheir`, `Abstract:WyrdnessBound` | Neutral Wyrd targets remain neutral to Wyrdness but take x0.90 Necrotic; authored native and exact reactions still win. Confirm pure Wyrd casting remains viable without erasing archetype-specific resistance. |
| 2 | Red Death infected | Implemented; validate in game | `Red Death`, `Infected`, `Abstract:Zombie`, `Abstract:Bloody` | Current rules resist Poison and Necrotic at x0.85, add Fire weakness only when vanilla has not already handled Fire, and add mild slash/pierce weaknesses while leaving blunt neutral. Confirm Red Death vanilla Poison/Fire multipliers are skipped rather than duplicated. |
| 2 | Flesh undead | Implemented cautiously; validate in game | `Zombie`, `Undead`, `Bloody`, `Frostbitten Warrior`, `Plaguewraith` | Current rules mildly resist blood/bleed/poison and pierce, strongly resist Necrotic at x0.60, leave slash neutral, and add modest Fire/blunt weakness. Wights are corrected to Flora; confirm DrownedZombie and InfectedFlesh terms refine broad FleshUndead metadata when present. |
| 2 | Spirits and shades | Implemented; validate in game | `Ghost`, `Banshee`, `Melancholy`, `Mist`, `Spirit`, `Wraith`, `HitMagic` | Current rules resist blood/bleed/poison and modestly resist physical; Wyrdness and x1.15 Necrotic are the strong answers. Confirm both sides are readable without becoming a hard wall or universal shortcut. |
| 2 | Sarras sea flesh | Implemented; validate in game | `Finbled`, `Tadpole`, `Tide`, `Floatling`, `Archivist`, `Scion`, `Reef`, `Type:SarrasCreature` | Current rules resist Cold, take x1.10 Necrotic, add modest Electric weakness, and share ordinary flesh's mild slash/pierce weaknesses while leaving blunt neutral. Confirm common vanilla `Cold 60%` rows are preserved by subtype skip logic. Exact Wailcaps retain these general rules but take x1.15 Necrotic through separate fungal-body evidence. |
| 3 | Flora and fungus | Implemented; validate in game | `Dryad`, `Frond`, `Tree`, `Stump`, `Fleshtree`, exact Wights and Wailcap fungal bodies | Flora resists Poison/Bleed/Pierce and takes x1.15 Necrotic plus Fire/Slash weakness. Wailcaps receive only the x1.15 fungal Necrotic matchup and otherwise remain SeaFlesh. |
...
| 4 | Ordinary flesh | Implemented cautiously; validate in game | `Wolf`, `Bear`, `Bandit`, `Outlaw`, `Human`, `Humanoid`, `Remor`, `Redcap`, `Corpse Eater` | Current rules add blood magic plus mild bleed/poison/slash/pierce bonuses only when no more specific family wins first. Exact Soul Rend adds x1.10 Necrotic without granting armored targets the separate generic spell bonus. |
...
| Verify Stagfather, Straw Dad/Son, Ghost of Broc Meala, Sleepwalker, Wights, Wailcaps, Wyrd Slimes, Frostgrot, Ancient Beholder, Giant, and Frostbitten Warriors with diagnostics enabled. | Confirm each corrected identity/body pair and ensure exact flesh targets no longer report `StoneBody` or receive the `Construct/Stone` arrow rule. |
...
| Validate neutral Wyrdness matchups on Wyrdspawn and Wyrdspirits. | `Abstract:WyrdnessBound` identifies the family, but vanilla does not apply a default Wyrdness multiplier to these templates; the removal of the blanket resistance should make specialist casting viable. |

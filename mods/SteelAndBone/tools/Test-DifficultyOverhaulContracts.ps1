...
Assert-Contract ($mainSource.Contains('PluginVersion = "4.2.6"')) "Plugin version is not 4.2.6."
Assert-Contract ($mainSource.Contains('AssemblyVersion("4.2.6.0")') -and $mainSource.Contains('AssemblyFileVersion("4.2.6.0")') -and $mainSource.Contains('AssemblyInformationalVersion("4.2.6")')) "Assembly version attributes are not 4.2.6."
Assert-Contract ($mainSource.Contains('ConfigSchemaVersion = 31')) "Config schema is not 31."
...
Assert-Contract ($mainSource.Contains('private bool TryResolveAxeMaterialRule(') -and $mainSource.Contains('ApplyPresetIntensity(1.20f)')) "The axe versus wood/flora rule is missing or not x1.20 at full material-rule intensity."
...
Assert-Contract ($mainSource.Contains('"Combat Rules", "Respect Native Multipliers", 20, 0')) "Combat Rules UI metadata is missing."
Assert-Contract ($mainSource.Contains('"Advanced - Native Multiplier Limits", "Maximum Amplified Weakness", 50, 10')) "Advanced native-multiplier UI metadata is missing."
Assert-Contract ($mainSource.Contains('"Preset - Combat and Materials", "Effectiveness Feedback Sensitivity", 11, 80')) "Effectiveness feedback sensitivity preset UI metadata is missing."
Assert-Contract ($mainSource.Contains('"Damage Numbers", "Maximum Active", 40, 150')) "Damage Numbers UI ordering is missing."
Assert-Contract ($mainSource.Contains('"EffectivenessFeedbackSensitivity", 1.10f')) "Effectiveness feedback sensitivity does not use the Hardened default."
Assert-Contract ($difficultySource.Contains('_effectivenessFeedbackSensitivity.Value = tuning.EffectivenessFeedbackSensitivity;')) "Applied presets do not set feedback sensitivity."
...
Assert-Contract ($difficultySource.Contains('"WeakSpotDamageBonus"') -and $difficultySource.Contains('_weakSpotDamageBonus.Value = tuning.WeakSpotDamageBonus;')) "Weak Spot Damage Bonus is not an applied preset value."
Assert-Contract ($difficultySource.Contains('WeakSpotDamageBonus = 0.10f,') -and $difficultySource.Contains('WeakSpotDamageBonus = 0.20f,') -and $difficultySource.Contains('WeakSpotDamageBonus = 0.30f,')) "Weak Spot Damage Bonus preset values are incomplete."
Assert-Contract ($difficultySource.Contains('ApplySelectedPreset();')) "Preset changes do not apply the centralized value template."
...
Assert-Contract ($mainSource.Contains('"Advanced - Target Families", "Armored Humanoid Terms", 51, 100')) "Target family UI ordering is missing."
...
Assert-Contract ([regex]::IsMatch($difficultySource, '"Preset - Enemies and Progression",\s*"Additional Enemy Attack Slots",\s*13,\s*40')) "Enemy attack-slot bonus UI metadata is missing."
Assert-Contract (-not $difficultySource.Contains('EnemyAttackSlotCap')) "Removed EnemyAttackSlotCap setting remains."
Assert-Contract ([regex]::IsMatch($difficultySource, '"Preset - Enemies and Progression",\s*"Hostile Archer Aim Scatter \(Meters\)",\s*13,\s*20')) "Hostile archer aim-scatter UI metadata is missing."
...
Assert-Contract ([regex]::IsMatch($difficultySource, 'PresetTenacityFactor\(\)[\s\S]{0,250}_tenacityFactor\.Value')) "Tenacity does not read the live applied value."
...
Assert-Contract ($difficultySource.Contains('_passiveShieldProtectionShare.Value') -and $difficultySource.Contains('_passiveShieldProtectionShare.Value = tuning.PassiveShieldProtectionShare;')) "Passive shield protection does not use the live applied value."
...
Assert-Contract ($difficultySource.Contains('_enemyMovementSpeedMultiplier.Value = tuning.EnemyMovementSpeedMultiplier;')) "Enemy movement multiplier is not part of the applied preset."
...
Assert-Contract ($difficultySource.Contains('_supportingPressure.Value') -and $difficultySource.Contains('_supportingPressure.Value = tuning.SupportingPressure;')) "Supporting pressure does not use the live applied value."
Assert-Contract ($difficultySource.Contains('_playerDamagePressure.Value') -and $difficultySource.Contains('_playerDamagePressure.Value = tuning.PlayerDamagePressure;')) "Player damage pressure does not use the live applied value."
...
Assert-Contract ($difficultySource.Contains('_combatManaRegenerationMultiplier.Value = tuning.PositiveBonusMultiplier;') -and $difficultySource.Contains('_positiveParryWindowBonusMultiplier.Value = tuning.PositiveBonusMultiplier;')) "Combat-sustainability multipliers are not part of the applied preset."
Assert-Contract ($difficultySource.Contains('_dashStaminaCostMultiplier.Value = tuning.DashStaminaCostMultiplier;')) "Dash stamina cost is not part of the applied preset."
...
Assert-Contract ($difficultySource.Contains('_enemyAttackSlotBonus.Value = tuning.EnemyAttackSlotBonus;')) "Enemy attack-slot bonus is not part of the applied preset."
...
Assert-Contract ([regex]::IsMatch($difficultySource, 'private float PresetPlayerPressureAmount\(\)[\s\S]{0,250}_playerDamagePressure\.Value')) "Player pressure does not read the live applied value."
...
Assert-Contract ([regex]::IsMatch($difficultySource, 'ApplyKillExperience[\s\S]*?ConfiguredExperienceMultiplier\(\)[\s\S]*?ApplyQuestExperience[\s\S]*?ConfiguredExperienceMultiplier\(\)[\s\S]*?ApplyProficiencyExperience[\s\S]*?ConfiguredExperienceMultiplier\(\)')) "Kill, quest, and proficiency experience do not all use the live applied multiplier."
...
Assert-Contract ([regex]::IsMatch($difficultySource, 'PresetArrowVelocityMultiplier\(\)[\s\S]{0,250}_arrowVelocityMultiplier\.Value')) "Arrow velocity does not read the live applied value."
Assert-Contract ($difficultySource.Contains('_hostileArcherAimScatter.Value = tuning.HostileArcherAimScatter;')) "Hostile archer aim scatter is not part of the applied preset."
...
Assert-Contract ([regex]::IsMatch($difficultySource, 'PresetEnemySightRangeMultiplier[\s\S]{0,250}_enemyAwarenessMultiplier\.Value')) "Enemy sight range does not read the live applied value."
Assert-Contract (-not $difficultySource.Contains("ModifyArmorPhysicalProtection")) "Removed physical armor protection setting is still present."
Assert-Contract (-not $difficultySource.Contains("PresetPhysicalArmorMultiplier")) "Removed physical armor multiplier helper is still present."
Assert-Contract (-not $difficultySource.Contains("ApplyPhysicalArmorProtection")) "Removed physical armor application method is still present."
Assert-Contract (-not $difficultySource.Contains("IsPhysicalDamageSubtype")) "Removed physical armor subtype helper is still present."
Assert-Contract (-not $difficultySource.Contains("HeroPhysicalArmorPatch")) "Removed physical armor patch is still present."
Assert-Contract (-not $difficultySource.Contains("Hero.TotalArmor")) "Removed Hero.TotalArmor hook is still present."
Assert-Contract ($difficultySource.Contains('return 1.0f + (PresetPenaltyAmount() * 0.5f);')) "Light armor mobility no longer follows half of the live supporting pressure."
...
Assert-Contract ([regex]::IsMatch($difficultySource, 'PresetEnemyHearingRangeMultiplier\(\)[\s\S]{0,150}return PresetEnemySightRangeMultiplier\(\);')) "Enemy hearing does not share the live awareness multiplier."
...
Assert-Contract ([regex]::IsMatch($difficultySource, 'PresetEnemyAggroPersistenceMultiplier\(\)[\s\S]{0,150}return PresetEnemySightRangeMultiplier\(\);')) "Enemy aggro persistence does not share the live awareness multiplier."
...
Assert-Contract ([regex]::IsMatch($difficultySource, 'PresetPotionPoisoningDecayPerSecond[\s\S]{0,350}_potionPoisoningWindowSeconds\.Value[\s\S]{0,100}20\.0f / windowSeconds')) "Potion Poisoning decay does not derive from the live configured window."
...
Assert-Contract ([regex]::IsMatch($difficultySource, 'PresetFoodHealthRateMultiplier[\s\S]{0,250}_foodHealthRateMultiplier\.Value')) "Food health rate does not read the live applied value."
Assert-Contract ([regex]::IsMatch($difficultySource, 'PresetFoodHealthDurationMultiplier[\s\S]{0,250}_foodHealthDurationMultiplier\.Value')) "Food health duration does not read the live applied value."
Assert-Contract ([regex]::IsMatch($difficultySource, 'PresetFoodStaminaRate[\s\S]{0,250}_foodStaminaPerSecond\.Value')) "Food stamina recovery does not read the live applied value."
...
Assert-Contract ($difficultySource.Contains('_preventFoodUseInCombat.Value = tuning.PreventFoodUseInCombat;')) "Food combat-use restriction is not part of the applied preset."
...
Write-Output "Steel and Bone 4.2.6 difficulty contracts passed."

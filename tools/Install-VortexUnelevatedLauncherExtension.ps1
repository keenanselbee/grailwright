...
$sourceRoot = Join-Path $PSScriptRoot 'vortex-extension\grailwright-unelevated-launcher'
$info = Get-Content -LiteralPath (Join-Path $sourceRoot 'info.json') -Raw | ConvertFrom-Json
$usesDefaultPluginsRoot = [string]::IsNullOrWhiteSpace($VortexPluginsRoot)
...
if ($usesDefaultPluginsRoot) {
    & (Join-Path $sourceRoot 'launch-foa-unelevated.ps1') -PrepareOnly | Out-Null
}
...
$temporary = Join-Path $VortexPluginsRoot ('.grailwright-launcher-extension-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $sourceRoot 'launch-foa-unelevated.ps1') -Destination $temporary

    foreach ($item in $existing) {
        $backupPath = Join-Path $VortexPluginsRoot ('.grailwright-launcher-extension-backup-' + [guid]::NewGuid().ToString('N'))
